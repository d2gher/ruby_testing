...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:drink) { described_class.new('beer')}
      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(drink.type).to eq('beer')
      end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:drink) { described_class.new('beer', 6)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(drink.full?).to be false
      end
    end
  end
end
