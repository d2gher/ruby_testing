...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      ...
      subject(:guess_wrong) { described_class.new(5, '2') }

      # remove the 'x' before running this test
      it 'is not game over' do
        expect(guess_wrong).to_not be_game_over
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game_six) { described_class.new(5, '5', 6) }
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 6 guesses!\n"
        expect { game_six.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
